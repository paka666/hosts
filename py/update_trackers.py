...
protocols = ["http:/", "https:/", "udp:/", "ws:/", "wss:/", "http://", "https://", "udp://", "ws://", "wss://"]
fixed_protos = {
    "http:/": "http://",
    "https:/": "https://",
    "udp:/": "udp://",
...
# First fix starting protocols if missing / or keep
...
# New step for concatenated protocols at start
new_cleaned = []
for t in cleaned:
    match = re.match(r'^((?:http|https|udp|ws|wss)://?)+(.+?)$', t)
    if match:
        protos_part = match.group(1)
        suffix = match.group(2)
        proto_matches = re.finditer(r'(http|https|udp|ws|wss)', protos_part)
...
for t in cleaned:
    ...
    while True:
        ...
        pos = len(current) + 1
        for proto in protocols:
            p = current.find(proto, 1)
            if 0 < p < pos:
                pos = p
                found_proto = proto
        if found_proto:
            first = current[:pos]
            rest = fixed_protos[found_proto] + current[pos + len(found_proto):]
...
try:
    ...
    tlds = set([
        ...
        'pro', 'tv', 'ws', 'mobi', 'asia', 'name', 'today', 'club', 'top', 'xyz', 'i2p'
    ])
