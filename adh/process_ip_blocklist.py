...
def extract_ips_from_line(line: str) -> set:
    ...
    line = line.split('%')[0]  # 移除zone ID
    try:
        if '/' in line:
            network = ipaddress.ip_network(line, strict=False)
            if network.prefixlen < 8:  # 过滤大网段
                print(f"过滤大网段: {network}")
                return set()
            return {network}
        else:
...
def recursive_collapse(networks: list, batch_size=50000) -> list:
    ...
    for batch in batches:
        collapsed_batches.extend(list(ipaddress.collapse_addresses(batch)))
    
    # 递归合并批次结果
    return recursive_collapse(collapsed_batches, batch_size)
...
def consolidate_networks(ip_list: set) -> list:
    ...
    ipv4_nets = sorted([net for net in network_objects if net.version == 4])
    ipv6_nets = sorted([net for net in network_objects if net.version == 6])
    
    print(f"IPv4网络数量: {len(ipv4_nets)}, IPv6网络数量: {len(ipv6_nets)}")
    
    collapsed_v4 = recursive_collapse(ipv4_nets)
    collapsed_v6 = recursive_collapse(ipv6_nets)
...
def main():
    ...
    ipv4_size = output_dir / 'ipv4.txt'
    ipv6_size = output_dir / 'ipv6.txt'
    ipv4_mb = ipv4_size.stat().st_size / 1024 / 1024 if ipv4_size.exists() else 0
    ipv6_mb = ipv6_size.stat().st_size / 1024 / 1024 if ipv6_size.exists() else 0
    
    print(f"最终输出: IPv4: {len(ipv4)} 条目 ({ipv4_mb:.2f} MB), IPv6: {len(ipv6)} 条目 ({ipv6_mb:.2f} MB)")
