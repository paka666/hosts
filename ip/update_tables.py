...
INBOUND_KEYWORDS = [
    ...
    'fullbogons', 'asn', 'geolite', 'country'
]
...
OUTBOUND_KEYWORDS = [
    ...
    'proxz', 'socks_proxy', 'sslproxies', 'xroxy'
]
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if any(file.endswith(ext) for ext in ip_extensions):
                ...
                ips = process_single_file(os.path.join(root, file))
                if any(keyword in file_name_lower for keyword in OUTBOUND_KEYWORDS):
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 入站完整列表
        with open('ip/inbound-blocklist.txt', 'w', encoding='utf-8') as f:
            for network in inbound_ipv4 + inbound_ipv6:
                f.write(str(network) + '\n')
        
        # 入站IPv4
        with open('ip/inbound-ipv4-list.txt', 'w', encoding='utf-8') as f:
            for network in inbound_ipv4:
                f.write(str(network) + '\n')
        
        # 入站IPv6
        with open('ip/inbound-ipv6-list.txt', 'w', encoding='utf-8') as f:
            for network in inbound_ipv6:
                f.write(str(network) + '\n')
        
        # 出站完整列表
        with open('ip/outbound-blocklist.txt', 'w', encoding='utf-8') as f:
            for network in outbound_ipv4 + outbound_ipv6:
                f.write(str(network) + '\n')
        
        # 出站IPv4
        with open('ip/outbound-ipv4-list.txt', 'w', encoding='utf-8') as f:
            for network in outbound_ipv4:
                f.write(str(network) + '\n')
        
        # 出站IPv6
        with open('ip/outbound-ipv6-list.txt', 'w', encoding='utf-8') as f:
            for network in outbound_ipv6:
                f.write(str(network) + '\n')
